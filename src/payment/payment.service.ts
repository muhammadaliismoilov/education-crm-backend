...
export class PaymentService {
  ...
  private async getEffectivePrice(studentId: string, groupId: string, groupPrice: number): Promise<number> {
  ...
  async create(dto: CreatePaymentDto) {
    ...
    try {
      ...
      await queryRunner.rollbackTransaction();
      if (err instanceof BadRequestException) throw err;
      throw new InternalServerErrorException("To'lov saqlashda xatolik yuz berdi");
    } finally {
      await queryRunner.release();
    }
  }
  ...
  async findAll(search?: string, page = 1, limit = 10) {
    ...
    if (studentGroupPairs.length > 0) {
      ...
      totals.forEach((row) => {
        totalPaidMap.set(`${row.studentId}_${row.groupId}`, Number(row.totalPaid || 0));
      });
    }

    // ✅ Discountlarni bitta query da olamiz
    const studentIds = [...new Set(items.map((p) => p.student?.id).filter(Boolean))];
    const discounts = studentIds.length > 0
    ...
    const formattedItems = items.map((payment) => {
      ...
      const discount = discounts.find(
        (d) => d.student?.id === payment.student?.id && d.group?.id === payment.group?.id,
      );
      const coursePrice = discount
    ...
    try {
      ...
      if (dto.amount !== undefined) {
        ...
        const totalMonthlyPrice = student?.enrolledGroups?.reduce((sum, g) => {
          ...
          return sum + (gDiscount ? Number(gDiscount.customPrice) : Number(g.price || 0));
        }, 0) || 0;

        await queryRunner.manager.update(Student, { id: studentId }, {
          balance: allTotalPaid - totalMonthlyPrice,
        });
      } else if (dto.paymentDate) {
        await queryRunner.manager.update(Payment, { id }, { paymentDate: dto.paymentDate });
      }
      ...
      if (err instanceof NotFoundException) throw err;
      if (err instanceof BadRequestException) throw err;
      throw new InternalServerErrorException("To'lovni yangilashda xatolik yuz berdi");
    } finally {
      await queryRunner.release();
    }
  }
  ...
  async remove(id: string) {
    ...
    try {
      ...
      return { success: true, message: "To'lov o'chirildi va balans to'g'rilandi" };
    } catch (err) {
      if (queryRunner.isTransactionActive)
        await queryRunner.rollbackTransaction();
      if (err instanceof NotFoundException) throw err;
      throw new InternalServerErrorException("To'lovni o'chirishda xatolik yuz berdi");
    } finally {
      await queryRunner.release();
    }
  }
