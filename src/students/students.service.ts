...
export class StudentsService {
  ...
  async update(
    ...
    // Student topilmasa — tmp faylni o'chir
    if (!student) {
      this.deleteFileIfExists(file?.path);
      throw new NotFoundException('Student topilmadi');
    }

    const { groupIds, pinfl, documentNumber, phone, discounts, ...updateData } =
      dto;
    ...
    // ─────────────────────────────────────────────
    // RASM YUKLASH (ixtiyoriy) — faqat file kelsa
    // ─────────────────────────────────────────────
    if (file) {
      ...
      try {
        ...
        // Descriptor olinmasa — tmp faylni o'chir
        this.deleteFileIfExists(file.path);
        throw new BadRequestException(
    ...
    try {
      ...
      // Agar xatolik bo'lsa va yangi fayl rename qilingan bo'lsa ham — o'chirmaydi
      // (chunk student save qilindi, lekin relation xatolik bo'lishi mumkin)
      if (error.code === '23505')
        throw new ConflictException(
          "Ma'lumotlar bazasida takrorlanish yuz berdi",
        );
